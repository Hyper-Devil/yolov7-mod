...
class ShuffleAttention(nn.Module):
    ...
    def __init__(self, channel=512, out_c=512, G=8):
        super().__init__()
        self.G=G
        self.channel=channel
...
class HorNetBlock(nn.Module):
    r""" HorNet block
    """
    def __init__(self, dim, dim_out, drop_path=0., layer_scale_init_value=1e-6, gnconv=gnconv):
        super().__init__()
...
class HorNet(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, 
                 ...
                 gnconv=gnconv, block=HorNetBlock, uniform_init=False, **kwargs
                 ):
        super().__init__()
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[block(dim=dims[i], dim_out=dims[i], drop_path=dp_rates[cur + j], 
